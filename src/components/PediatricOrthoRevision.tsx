...
const PediatricFemurRevision = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans pb-20 transition-colors duration-300">
      {/* Utility Bar */}
      ...
      <header className="max-w-6xl mx-auto p-4 md:p-8 mb-10 border-b-2 border-slate-200 pb-8 flex flex-col md:flex-row justify-between items-end gap-4 transition-colors">
        <div>
          ...
          <p className="text-slate-500 font-bold text-xs mt-2 uppercase tracking-[0.3em] flex items-center gap-2 transition-colors">
            <Baby size={16} className="text-blue-500" /> High-Yield Surgical Revision 2026
          </p>
        </div>
        <nav className="flex bg-slate-200 p-1.5 rounded-2xl transition-colors">
          {Object.keys(ageGroups).map((age) => (
            <button
              key={age}
              onClick={() => setActiveAge(age)}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${activeAge === age ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              {age}
            </button>
          ))}
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-200 relative overflow-hidden transition-colors">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest transition-colors">Protocol for</span>
                <h2 className="text-2xl font-black text-slate-800 uppercase italic leading-none transition-colors">{ageGroups[activeAge as keyof typeof ageGroups].title}</h2>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div className="space-y-4">
                ...
                <p className="text-2xl font-black text-slate-900 tracking-tight transition-colors">{ageGroups[activeAge as keyof typeof ageGroups].treatment}</p>
                <p className="text-sm text-slate-500 font-medium leading-relaxed transition-colors">{ageGroups[activeAge as keyof typeof ageGroups].detail}</p>
              </div>
              <div className="p-6 bg-amber-50 rounded-3xl border border-amber-200 flex flex-col justify-center transition-colors font-sans">
                <div className="flex items-center gap-2 text-amber-700 mb-2 transition-colors">
                  <AlertCircle size={18} />
                  <h4 className="text-[10px] font-black uppercase tracking-widest">Technique Warning</h4>
                </div>
                <p className="text-xs font-bold text-amber-900 leading-relaxed italic transition-colors">
            ...
            {activeAge === '6m - 5y' && (
              <div className="p-6 bg-slate-900 rounded-[2rem] text-white transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm transition-colors">
              <div className="flex items-center gap-2 text-rose-600 mb-6 transition-colors">
                <Activity size={20} />
                <h3 className="text-sm font-black uppercase">Muscle Deformity Forces</h3>
              </div>
              <div className="space-y-4">
                <div className="flex gap-4">
                  <div className="p-2 bg-slate-100 rounded-lg text-rose-600 font-black text-xs h-fit uppercase transition-colors">Prox</div>
                  <p className="text-xs font-bold text-slate-700 leading-relaxed transition-colors">
                    <span className="text-rose-600 underline decoration-2">Iliopsoas</span>: Flexion and External Rotation.
                  </p>
                </div>
                <div className="flex gap-4">
                  <div className="p-2 bg-slate-100 rounded-lg text-slate-600 font-black text-xs h-fit uppercase transition-colors">Dist</div>
                  <p className="text-xs font-bold text-slate-700 leading-relaxed transition-colors">
                    <span className="text-slate-900 underline decoration-2">Adductors</span>: Shortening and Varus.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-xl transition-colors">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm transition-colors">
            ...
            <ul className="space-y-3">
              <li className="flex justify-between items-center text-xs">
                ...
                <span className="font-black text-slate-800 uppercase italic">Anterior</span>
              </li>
              <li className="flex justify-between items-center text-xs border-t border-slate-50 pt-2">
                <span className="font-bold text-slate-500">Narrowest Part</span>
                <span className="font-black text-slate-800 uppercase italic">Isthmus</span>
              </li>
              <li className="flex justify-between items-center text-xs border-t border-slate-50 pt-2">
                <span className="font-bold text-slate-500">Distribution</span>
                <span className="font-black text-slate-800 uppercase italic">Bimodal (2-4y & Adolescents)</span>
              </li>
            </ul>
          </div>

          <div className="bg-indigo-50 p-6 rounded-[2rem] border-2 border-indigo-200 transition-colors">
            <h4 className="text-[10px] font-black text-indigo-600 uppercase mb-2 tracking-widest">Stability Definition</h4>
            <div className="space-y-2">
              <div className="flex gap-2 text-[10px] font-bold text-indigo-900">
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mb-10 transition-colors">Select a high-yield topic to begin your revision</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <h3 className="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-2 transition-colors">DDH</h3>
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider transition-colors">Developmental Dysplasia of the Hip</p>
            ...
            <button 
              ...
              className="group flex flex-col p-8 bg-white border border-slate-200 rounded-[2.5rem] text-left hover:shadow-2xl hover:border-emerald-500 transition-all"
            >
              ...
              <h3 className="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-2 transition-colors">Femur</h3>
