...
const PCLInfographic = ({ onBack, onPractice }: PCLInfographicProps) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 font-sans text-slate-900 transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {onPractice && (
            <button 
              onClick={onPractice}
              className="flex items-center gap-2 px-5 py-2 bg-indigo-600 text-white text-xs font-black rounded-xl uppercase hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
            >
              <BookOpen size={14} />
              Practice Sports Med
            </button>
        ...
        {/* Main Title Card */}
        <div className="bg-white rounded-[2.5rem] shadow-xl overflow-hidden mb-12 border border-slate-200 transition-colors">
          <div className="bg-indigo-900 p-10 text-white relative border-b border-indigo-800 transition-colors">
            ...
            <div className="relative z-10">
               ...
               <p className="text-indigo-200 text-lg font-bold">Posterior Cruciate Ligament • Clinical Decision Support</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-4 space-y-3">
                {Object.entries(content).map(([key, section]) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-xl border border-slate-100 ${activeTab === key ? 'bg-white shadow-sm' : 'bg-slate-100'}`}>
                        {section.icon}
                      </div>
                ...
                <motion.div 
                  ...
                  className="mt-10 p-6 bg-rose-50 border border-rose-100 rounded-3xl"
                >
                  <div className="flex items-center gap-2 text-rose-700 font-black text-xs uppercase mb-3 tracking-widest">
                    <AlertCircle className="w-5 h-5 shrink-0" />
                    <span>Red Flag</span>
                  </div>
                  <p className="text-xs text-rose-800/80 italic font-bold leading-relaxed">
              ...
              <div className="lg:col-span-8">
                <motion.div 
                  ...
                  className="bg-slate-50/50 rounded-[2.5rem] p-8 md:p-10 border border-slate-100 min-h-[500px] transition-colors"
                >
                  <div className="flex items-center gap-4 mb-10">
                    <div className={`p-4 rounded-2xl bg-white shadow-sm border border-slate-100 ${content[activeTab as keyof typeof content].color}`}>
                  ...
                  <div className="grid gap-4">
                    {content[activeTab as keyof typeof content].items.map((item, i) => (
                      <div key={i} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-indigo-300 transition-all group">
                        <div className="flex flex-col">
                          <span className={`text-[10px] font-black uppercase tracking-[0.2em] mb-2 ${content[activeTab as keyof typeof content].color}`}>
                            {item.label}
                          </span>
                          <span className="text-slate-700 text-lg font-bold leading-snug transition-colors">
                            {item.text}
                          </span>
                        </div>
                      </div>
                  ...
                  {activeTab === 'grading' && (
                    ...
                    >
                      {[1, 2, 3].map(grade => (
                        <div key={grade} className="flex-1 p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                          <div className={`text-[10px] font-black uppercase tracking-widest mb-3 ${grade === 3 ? 'text-rose-600' : 'text-slate-400'}`}>Grade {grade}</div>
                          <div className={`h-2.5 rounded-full mb-3 ${grade === 1 ? 'bg-emerald-400' : grade === 2 ? 'bg-amber-400' : 'bg-rose-500'}`} />
                          <div className="text-xs font-black text-slate-700 uppercase italic">{grade === 1 ? 'Mild' : grade === 2 ? 'Moderate' : 'Severe'}</div>
                        </div>
                      ))}
                    </motion.div>
                  )}
        ...
        <footer className="text-center">
           <div className="inline-block px-10 py-3 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-[0.5em] shadow-xl transition-colors">
