...
export default function QuestionBank({ category, studyMode = false }: QuestionBankProps) {
  ...
  if (!currentQuestion && !isFinished) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
        <div className="w-20 h-20 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-3xl flex items-center justify-center mb-6 shadow-xl transition-colors">
          <CircleDashed className="w-10 h-10 text-indigo-500 animate-spin-slow" />
        </div>
        <h2 className="text-2xl font-black text-gray-900 dark:text-white italic uppercase tracking-tighter mb-2 transition-colors">Clinical Content Unavailable</h2>
        <p className="text-gray-500 dark:text-slate-400 font-bold text-sm uppercase tracking-widest max-w-xs transition-colors">
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 flex flex-col items-center">
        <motion.div 
          ...
          className="w-full bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-2xl shadow-indigo-100/50 dark:shadow-none text-center relative overflow-hidden transition-colors"
        >
          {/* Background Aura */}
          <div className="absolute top-0 left-0 w-full h-full bg-indigo-50 dark:bg-indigo-500/5 blur-[100px] pointer-events-none opacity-50" />
          
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-3xl flex items-center justify-center mb-6 border border-indigo-100 dark:border-indigo-500/20 shadow-inner transition-colors">
              <Trophy className="w-10 h-10" />
            </div>
            
            <h2 className="text-4xl font-black text-gray-900 dark:text-white italic uppercase tracking-tighter mb-2 transition-colors">Clinical Validation Complete</h2>
            <p className="text-gray-400 dark:text-slate-500 font-black text-xs uppercase tracking-[0.3em] mb-12 transition-colors">Performance Summary</p>
            
            <div className="grid grid-cols-3 gap-6 w-full mb-12">
              <div className="bg-gray-50 dark:bg-slate-950/50 border border-gray-100 dark:border-slate-800 p-6 rounded-3xl transition-colors">
                <span className="block text-4xl font-black text-gray-900 dark:text-white mb-1 transition-colors">{score}</span>
                <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest transition-colors">Correct</span>
              </div>
              ...
              <div className="bg-gray-50 dark:bg-slate-950/50 border border-gray-100 dark:border-slate-800 p-6 rounded-3xl transition-colors">
                <span className="block text-4xl font-black text-gray-900 dark:text-white mb-1 transition-colors">{questions.length}</span>
                <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest transition-colors">Total</span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full">
              <button 
                ...
                className="flex-1 flex items-center justify-center gap-3 px-8 py-5 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 rounded-2xl font-black uppercase tracking-widest transition-all text-[11px]"
              >
                <BookOpen size={20} />
                Detailed Review
              </button>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          <button 
            ...
            className="flex items-center gap-2 text-gray-400 dark:text-slate-500 hover:text-gray-900 dark:hover:text-white font-black uppercase tracking-widest text-[10px] transition-colors"
          >
            <ArrowLeft size={16} />
            Back to Results
          </button>
          <h2 className="text-xl font-black text-gray-900 dark:text-white italic uppercase tracking-tight transition-colors">Clinical Review Mode</h2>
        </div>
        ...
        <div className="space-y-6">
          {userAnswers.map((item, idx) => (
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2rem] p-8 shadow-sm overflow-hidden relative transition-colors"
            >
              <div className={`absolute left-0 top-0 w-2 h-full ${item.isCorrect ? 'bg-emerald-500' : 'bg-rose-500'}`} />
              <div className="flex gap-6">
                <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-black ${item.isCorrect ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 border border-emerald-100 dark:border-emerald-500/20' : 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-500 border border-rose-100 dark:border-rose-500/20'} transition-colors`}>
                  {idx + 1}
                </div>
                <div className="space-y-4 flex-1">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-slate-950 rounded-2xl border border-gray-100 dark:border-slate-800 transition-colors">
                      <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-1 transition-colors">Peer consensus</span>
                  ...
                  {item.explanation && (
                    <div className="p-6 bg-indigo-50/50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/10 rounded-2xl transition-colors">
                      <h4 className="flex items-center gap-2 text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-3 transition-colors">
                        <Info size={14} /> Clinical Rationale
                      </h4>
                      <p className="text-sm text-indigo-900/70 dark:text-slate-400 leading-relaxed font-medium transition-colors">{item.explanation}</p>
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-[2.5rem] p-8 mb-8 shadow-xl shadow-gray-200/50 dark:shadow-none relative overflow-hidden backdrop-blur-xl transition-colors">
        <div className="flex items-start justify-between relative z-10 mb-8">
          <div className="space-y-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex items-center gap-1.5 text-gray-400 dark:text-slate-500 transition-colors">
                <Activity size={14} className="text-emerald-500" />
                <span className="text-[10px] font-black uppercase tracking-widest">Live Clinical Rotation</span>
              </div>
            </div>
            <h2 className="text-2xl font-black text-gray-900 dark:text-white italic uppercase tracking-tighter mb-2 transition-colors">
          ...
          <div className="flex flex-col items-end gap-2">
             {!studyMode ? (
               <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-inner transition-colors">
                  <Clock size={16} className={`${timeLeft <= 5 ? 'text-rose-500 animate-pulse' : 'text-indigo-600'}`} />
                  <span className={`text-xl font-black font-mono tracking-tighter ${timeLeft <= 5 ? 'text-rose-500' : 'text-gray-900 dark:text-white'} transition-colors`}>
                    {timeLeft.toString().padStart(2, '0')}<span className="text-xs text-gray-400 dark:text-slate-600 ml-1 transition-colors">SEC</span>
                  </span>
               </div>
             ) : (
               <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 rounded-2xl transition-colors">
                  <BookOpen size={16} className="text-amber-600" />
                  <span className="text-xs font-black text-amber-600 uppercase tracking-widest">Study Mode</span>
               </div>
             )}
             <span className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest pr-2 transition-colors">Case {currentIndex + 1} of {questions.length}</span>
          </div>
        </div>

        {/* Progress Bar */}
        <div className="relative h-2 w-full bg-gray-100 dark:bg-slate-950 rounded-full overflow-hidden mb-2 transition-colors">
          <motion.div 
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${!anySelected ? 'bg-white dark:bg-slate-900 border-gray-100 dark:border-slate-800 hover:border-indigo-600 hover:shadow-xl hover:shadow-indigo-100/50 dark:hover:shadow-none active:scale-[0.98]' : ''}
                    ${isSelected ? (isCorrect ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-500 dark:border-emerald-500 ring-4 ring-emerald-500/10' : 'bg-rose-50 dark:bg-rose-500/10 border-rose-500 dark:border-rose-500 ring-4 ring-rose-500/10') : ''}
                    ${anySelected && isCorrect && !isSelected ? 'bg-emerald-50/50 dark:bg-emerald-500/5 border-emerald-500/20' : ''}
                    ${anySelected && !isSelected && !isCorrect ? 'bg-white dark:bg-slate-900 border-gray-50 dark:border-slate-800 opacity-40 scale-95' : ''}
                  `}
                >
                  <div className="flex items-center gap-6">
                    <div className={`
                      w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm transition-colors border-2
                      ${!anySelected ? 'bg-gray-50 dark:bg-slate-950 border-gray-100 dark:border-slate-800 text-gray-400 group-hover:border-indigo-600 group-hover:bg-indigo-600 group-hover:text-white' : ''}
                    ...
                    <span className={`text-xl font-bold ${anySelected ? (isCorrect ? 'text-emerald-700 dark:text-emerald-500' : isWrong ? 'text-rose-700 dark:text-rose-500' : 'text-gray-400 dark:text-slate-600') : 'text-gray-700 dark:text-slate-300'} transition-colors`}>
                      {option}
                    </span>
                  </div>
      ...
      {/* Action Bar */}
      <div className="mt-12 flex items-center justify-between pt-10 border-t border-gray-200 dark:border-slate-800">
        ...
        <button
          ...
          className={`
            ...
            ${selectedOption !== null 
              ? 'bg-indigo-600 dark:bg-white text-white dark:text-slate-950 shadow-xl shadow-indigo-600/20 dark:shadow-none' 
              : 'bg-gray-100 dark:bg-slate-900 text-gray-300 dark:text-slate-700 cursor-not-allowed'}
          `}
        >
      ...
      <AnimatePresence>
        {showExplanation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="mt-8 p-10 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/50 dark:shadow-none relative overflow-hidden transition-colors"
          >
            <div className="absolute top-0 right-0 p-8 opacity-5">
              <BookOpen size={80} className="text-indigo-600" />
            </div>
            <h4 className="flex items-center gap-3 text-xs font-black text-indigo-600 dark:text-indigo-400 mb-6 uppercase tracking-[0.3em] relative z-10 transition-colors">
